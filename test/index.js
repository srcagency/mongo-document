...
Mongo.Driver.configure('default', { database: 'test' });
var db = new Mongo.Database('default');
...
function getModel() {
	...
	Person.collection = db.collection('models_' + (counter++) + (new Date()).getTime().toString())
		.then(function(c){
			return c;
		});
...
test('statics', function( t ){
	...
	t.test('findAndModify', function( t ){
		...
		var update = saved
			.then(function(){
				return Person.findAndModify({ name: 'Eve' }, null, { $set: { age: 10 } });
			})
			.tap(function( eve ){
